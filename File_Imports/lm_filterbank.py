...
def makeLMfilters():
    ...
    Constructs the 48 LM filters.
    
    INPUTS:
      None
      
    OUTPUTS:
      F: filter_set as shape (49,49,48) where the i-th filter is in the last dimension (F.shape[3])
      
    EXAMPLE USE:
      F = makeLMfilters()
      print(F.shape)
      
      18 First order Gaussian Filters:
      for i in range(0,18):
      plt.subplot(3,6,i+1)
      plt.axis('off')
      plt.imshow(F[:,:,i], cmap = 'gray')
      
      18 Second order Gaussian Filters:
      for i in range(0,18):
      plt.subplot(3,6,i+1)
      plt.axis('off')
      plt.imshow(F[:,:,i+18], cmap = 'gray')
      
      12 Gaussian and Laplacian Filters:
      for i in range(0,12):
      plt.subplot(4,4,i+1)
      plt.axis('off')
